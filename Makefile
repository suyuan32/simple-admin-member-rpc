...
docker:
	docker build -f Dockerfile -t ${DOCKER_USERNAME}/mms-rpc:${VERSION} .
...
publish-docker:
	...
	docker push ${DOCKER_USERNAME}/mms-rpc:${VERSION}
	@printf $(GREEN)"[SUCCESS] publish docker successfully"

gen-rpc:
	goctls rpc protoc ./mms.proto --go_out=. --go-grpc_out=. --zrpc_out=.
...
gen-rpc-ent-logic:
	goctls rpc ent --schema=./ent/schema  --style=go_zero --multiple=false --service_name=mms --search_key_num=3 --o=./ --model=$(model) --group=$(group) --proto_out=./desc/$(shell echo $(model) | tr A-Z a-z).proto
